...
#  HARDENED MINIMAL PATCH:
#   - disables root SSH password login (key-only; ssh disabled if no key)
#   - binds code-server to localhost and uses random password
...
get_config() {
  ...
  read -rp "VLAN tag (press Enter for none): " CT_VLAN
  if [[ -n "$CT_VLAN" ]]; then
    [[ "$CT_VLAN" =~ ^[0-9]+$ ]] || error "VLAN tag must be a number."
    (( CT_VLAN >= 1 && CT_VLAN <= 4094 )) || error "VLAN tag must be between 1 and 4094."
  fi

  # Security posture: Docker-in-LXC requires privileged + unconfined on many setups.
  # Make this explicit so the user opts in.
  read -rp "Enable Docker-in-LXC mode (privileged + AppArmor unconfined)? (y/N): " CT_DOCKER_LXC
  CT_DOCKER_LXC="${CT_DOCKER_LXC:-N}"
  ...
  echo "  Network:    $CT_IP"
  echo "  DNS:        $CT_DNS"
  [[ -n "${CT_VLAN:-}" ]] && echo "  VLAN:       $CT_VLAN"
  echo "  Docker-LXC:  $CT_DOCKER_LXC"
  echo "─────────────────────────────────────────────────"
  echo ""
...
provision_container() {
  ...
  PROVISION_TMP="$(mktemp -p /tmp "provision-${CT_ID}.XXXXXX.sh")"
  export PROVISION_TMP
  cat > "$PROVISION_TMP" << 'PROVISION_EOF'
#!/bin/bash
set -e
...
echo ">>> Creating non-root user (claude) for interactive work..."
id -u claude >/dev/null 2>&1 || useradd -m -s /bin/bash claude
usermod -aG sudo claude >/dev/null 2>&1 || true

echo ">>> Setting up SSH (key-only; disable if no keys present)..."
mkdir -p /etc/ssh
...
echo ">>> Ensuring SSH is only enabled if an authorized_keys exists..."
if [[ -f /root/.ssh/authorized_keys ]] && [[ -s /root/.ssh/authorized_keys ]]; then
  systemctl enable ssh
  systemctl restart ssh
  echo "    SSH enabled (keys found)."
else
  systemctl disable ssh >/dev/null 2>&1 || true
  systemctl stop ssh >/dev/null 2>&1 || true
  echo "    SSH disabled (no keys found). Use pct enter to access."
fi
...
PROVISION_EOF
  ...
  # Push provision script and a random code-server password into the container
  chmod +x "$PROVISION_TMP"
  CODE_SERVER_PASSWORD="$(openssl rand -base64 24 | tr -d '\n' || true)"
  [[ -n "$CODE_SERVER_PASSWORD" ]] || CODE_SERVER_PASSWORD="change-me-$(date +%s)"
  pct push "$CT_ID" "$PROVISION_TMP" /tmp/provision.sh
  pct exec "$CT_ID" -- chmod +x /tmp/provision.sh
  pct exec "$CT_ID" -- bash -lc "export CODE_SERVER_PASSWORD='${CODE_SERVER_PASSWORD}'; /tmp/provision.sh"
  rm -f "$PROVISION_TMP"
}
...
print_summary() {
  ...
  [[ -n "${ct_ip:-}" ]] && echo -e "    Code:     ${CYAN}ssh -L 8443:127.0.0.1:8443 root@${ct_ip}${NC}  then open http://127.0.0.1:8443"
  echo ""
